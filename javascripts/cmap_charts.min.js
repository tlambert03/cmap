...
async function makeHSLChart(canvas,data){var datasets=[];var label_data,_data,val;["hue","saturation","chroma"].forEach((label)=>{label_data=data[label];_data=[];for(i=0;i<data.x.length;i++){val=label_data[i];_data.push({x:data.x[i],y:label=="hue"?(val*10)/36:val,});}
datasets.push({label:label,showLine:true,data:_data});});new Chart(canvas,{type:"scatter",data:{datasets:datasets},options:{...GLOBAL_OPTIONS,plugins:{legend:{display:true,labels:{boxHeight:1}}},scales:{},elements:{line:{borderWidth:4},point:{radius:0},},},});}